...
import com.cashiroai.shared.data.SharedDataGraph
...
import com.cashiroai.shared.domain.usecase.TransactionMutationResult
import com.cashiroai.shared.domain.usecase.UpdateManualTransactionUseCase
...
class CashiroSharedFacade {
    ...
    fun addExpenseAndLoadHome(
        ...
        addTransactionAndLoadHome(
            ...
            occurredAtEpochMillis = com.cashiroai.shared.data.util.currentTimeMillis(),
            bankName = null,
            accountLast4 = null
        )
    }
    ...
    fun importStatementTextAndLoadHome(statementText: String): SharedHomeSnapshot = safeSnapshot {
        runBlocking {
            ...
            val debugParser = com.cashiroai.shared.data.statement.SharedStatementParserFactory.getParser(statementText)
            if (debugParser != null) {
                val debugParsed = debugParser.parse(statementText)
                debugParsed.take(3).forEachIndexed { i, tx ->
                    println("[Cashiro DEBUG] Tx #${i + 1}: merchant=${tx.merchant}, amount=${tx.amountMinor}, timestamp=${tx.timestampEpochMillis}, type=${tx.transactionType}, bank=${tx.bankName}")
                }
            }
        ...
        return try {
            runBlocking {
                graph.transactionRepository.softDelete(
                    transactionId,
                    com.cashiroai.shared.data.util.currentTimeMillis()
                )
                true
            }
        } catch (_: Throwable) {
        ...
        return try {
            runBlocking {
                ...
                graph.categoryRepository.insertCategory(
                    com.cashiroai.shared.data.model.SharedCategory(
                        name = name,
                        colorHex = colorHex,
                        isSystem = false,
                        isIncome = isIncome,
        ...
        return try {
            runBlocking {
                ...
                graph.accountRepository.insertBalance(
                    com.cashiroai.shared.data.local.entity.SharedAccountBalanceEntity(
        ...
        return try {
            runBlocking {
                ...
                graph.accountRepository.upsertCard(
                    com.cashiroai.shared.data.local.entity.SharedCardEntity(
    ...
    fun addSubscriptionAndLoadHome(
        ...
        runBlocking {
            val now = com.cashiroai.shared.data.util.currentTimeMillis()
        ...
        return try {
            runBlocking {
                ...
                graph.ruleRepository.upsertRule(
                    com.cashiroai.shared.data.local.entity.SharedRuleEntity(
                        id = id,
                        name = name.trim(),
    ...
    private fun com.cashiroai.shared.data.local.entity.SharedRuleEntity.toRuleItem() =
        SharedRuleItem(
            id = id,
            name = name,
            conditionsJson = conditionsJson,
    ...
    private fun parseType(value: String): com.cashiroai.shared.data.model.SharedTransactionType {
        return com.cashiroai.shared.data.model.SharedTransactionType.entries.firstOrNull {
            it.name.equals(value, ignoreCase = true)
        } ?: com.cashiroai.shared.data.model.SharedTransactionType.EXPENSE
    }
    ...
    private fun com.cashiroai.shared.data.model.SharedTransaction.toItem() =
    ...
    private fun com.cashiroai.shared.data.local.entity.SharedAccountBalanceEntity.toAccountItem() =
        SharedAccountItem(
            bankName = bankName,
            accountLast4 = accountLast4,
            balanceMinor = balanceMinor,
    ...
    private fun com.cashiroai.shared.data.local.entity.SharedCardEntity.toCardItem() =
        SharedCardItem(
            id = id,
            cardLast4 = cardLast4,
            cardType = cardType,
    ...
    private fun com.cashiroai.shared.data.model.SharedCategory.toCategoryItem() =
        SharedCategoryItem(
            id = id,
            name = name,
            colorHex = colorHex,
